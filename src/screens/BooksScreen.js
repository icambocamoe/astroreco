...
export default function BooksScreen({ route, onFavorite }) {
  ...
  const { docRef, books, favoriteBooks, setFavoriteBooks } =
  ...
  const renderItem = (item, index) => {
    ...
    return (
      <View key={index} style={[styles.card, {borderWidth: 1, borderColor: themeData.texto, backgroundColor: themeData.vistas}]}>
        <Text style={[styles.title, dynamicStyles.dynamicText]}>{item.title}</Text>
        <View style={styles.row}>
          <Image source={{ uri: item.imgUrl }} style={styles.image} />
          <View style={styles.column}>
            <Text style={[dynamicStyles.dynamicText]}>Author: {item.author}</Text>
            <Text style={[dynamicStyles.dynamicText]}>Genre: {item.genre} </Text>
            <Text style={[dynamicStyles.dynamicText]}>Format: {item.format}</Text>
            <Text></Text>
            <TouchableOpacity style={{alignItems: "center"}} onPress={() => handleFavoritePress(item)}>
              <Ionicons
          ...
          {Object.keys(books || {}).length > 0 ? (
            Object.keys(books).map((category, catIndex) => (
              <View key={catIndex}>
                ...
                {Array.isArray(books[category]?.books) &&
                books[category].books.length > 0 ? (
